...
Qwen3-VL target with DFlash draft (text-only or text+image).
...
# Text-only input
messages = [{"role": "user", "content": [{"type": "text", "text": "How many positive whole-number divisors does 196 have?"}]}]

# Text+image input
# image_input = "/path/to/image.jpg"  # path / URL / PIL image
# messages = [{"role": "user", "content": [{"type": "image", "image": image_input}, {"type": "text", "text": "Describe this image."}]}]
...
output_text = processor.batch_decode(output[:, inputs["input_ids"].shape[1]:], skip_special_tokens=True)
print(output_text[0])
```
...
**Transformers** (Qwen3-VL target):
```bash
torchrun --nproc_per_node=8 -m dflash.benchmark --backend transformers \
    --model Qwen/Qwen3-VL-4B-Instruct --draft-model z-lab/Qwen3-8B-DFlash-b16 \
